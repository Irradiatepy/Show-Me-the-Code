...
class DetailProfile_Db(object):
	...
	def insertIntoDB(self, obj):
		...
		if self.findExists(obj.weiboid) == 0:
			self.cursor.execute('replace into detailprofile(weiboid, gender, description, address, birthday, blog, ' + 
					'relation, sexuality, bloodtype, fashion) values(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)', \
					(obj.weiboid, obj.gender, obj.description, obj.address, obj.birthday, obj.blog, obj.relation, \
					obj.sexuality, obj.bloodtype, obj.fashion))
		self.conn.commit()
